...
void MPCController::configure(
  ...
  declare_parameter_if_not_declared(node, plugin_name_ + ".delay_mode",
                                    rclcpp::ParameterValue(false));
  declare_parameter_if_not_declared(node, plugin_name_ + ".thread_numbers",
                                    rclcpp::ParameterValue(2));
  declare_parameter_if_not_declared(node, plugin_name_ + ".max_speed",
                                    rclcpp::ParameterValue(0.5));
  declare_parameter_if_not_declared(node, plugin_name_ + ".waypoints_dist",
                                    rclcpp::ParameterValue(-1.0));
  declare_parameter_if_not_declared(node, plugin_name_ + ".pathLength",
                                    rclcpp::ParameterValue(8.0)); // unit: m
  declare_parameter_if_not_declared(node, plugin_name_ + ".goal_radius",
                                    rclcpp::ParameterValue(0.5)); // unit: m
  declare_parameter_if_not_declared(node, plugin_name_ + ".controller_freq",
...
geometry_msgs::msg::TwistStamped MPCController::computeVelocityCommands(
  ...
  if (delay_mode_) {
    ...
    const double theta_act =
        w * dt; //(steering) theta_act = v * steering * dt / Lf;
...
Eigen::VectorXd MPCController::polyfit(Eigen::VectorXd xvals,
...
PLUGINLIB_EXPORT_CLASS(nav2_mpc_controller::MPCController,
